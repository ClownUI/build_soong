...
def parse_args():
  ...
  config["ClownDesc"] = config["BuildDesc"]
  config["ClownDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['ClownDesc']}")
  ...
  print(f"ro.clown.device={config['ClownDevice']}")
